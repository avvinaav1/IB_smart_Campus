...
5. **Review & deliver** lets you pick the event the certificates belong to (or leave it standalone), map email/username columns, preview variable email copy, inspect internal matches and select email and/or profile/chat delivery. The final button enqueues the explicitly selected actions and an archive. Every certificate email is wrapped in icebrkr branding; picking an event also adds that event's cover banner above the message and unlocks `{{event}}` as a subject/message variable (its title). The icebrkr wordmark (`public/logo.jpeg`) is embedded as a CID attachment, so it renders regardless of deployment state. The event banner is not — it's organizer-uploaded content served from the app itself, so it needs a real, reachable URL: set `APP_BASE_URL` in `.env.local` if your deployment's public origin differs from the production domain, and make sure whatever origin you send from has actually been deployed (a banner emailed from a build that hasn't been pushed live yet will 404 in the recipient's inbox).
...
Certificate emails never attach the rendered PNG. Instead, every emailed row that wasn't already internally delivered gets a pending claim record (`certificateClaims`, keyed by job+row so retries don't duplicate it), and the email itself tells the recipient to create an account or sign in with that same address. The next time someone registers, or logs in via OTP or password, with a matching email, `claimPendingCertificates` (`lib/certificates/claims.ts`) awards every pending claim for that address into their profile and Certificates inbox — the same transaction shape as `internalAward`, minus the live job lease, since the batch is typically long finished by claim time. The image itself is served only through the authenticated `/{certificateId}/image` route once claimed; the public `/verify` lookup by code still returns metadata only (name, course, issuer, date), never the image.
...
`certificateClaims` holds pending claim records (see above): one doc per emailed row that wasn't already internally delivered, deleted only implicitly by being marked `claimed: true`. Unclaimed records are not garbage-collected either; include them in the same retention job.
