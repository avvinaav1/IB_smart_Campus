...
import { Check, Download, ExternalLink, KeyRound, LoaderCircle, MapPin, Search, ShieldCheck, TicketCheck, UserPlus, Users, X } from "lucide-react";
...
export function EventRegistrationDetail({ event, close, notify, onChange }: { event: CampusEvent; close: () => void; notify: (message: string) => void; onChange: (event: CampusEvent) => void }) {
  ...
  return <div className="overlay" onMouseDown={mouse => mouse.target === mouse.currentTarget && close()}><section className={`event-modal ${adminOpen ? "admin-mode" : ""}`} role="dialog" aria-modal="true" aria-label={event.title}><div className="event-modal-image"><Image src={event.imageUrl} alt="" fill sizes="760px" unoptimized={event.imageUrl.startsWith("/api/")} /><button className="icon-button" type="button" onClick={close} aria-label="Close"><X size={20} /></button><span>{event.isCreator ? "YOUR EVENT" : event.isEventAdmin ? "YOU’RE AN ADMIN" : event.category}</span></div><div className="event-modal-copy"><span className="eyebrow pink">{event.month} {event.day} · {event.time}</span><h2>{event.title}</h2><p className="event-location"><MapPin size={18} /><span><b>{event.venueName}</b><small>{event.venueAddress} · {event.campus}</small></span>{event.directionsUrl && <a href={event.directionsUrl} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Directions</a>}</p><p>{event.description || "The details are set. Bring your campus energy and show up for the people making it happen."}</p><div className="capacity-row"><div><Users size={19} /><span><b>{event.going} going</b><small>{Math.max(0, event.capacity - event.going)} spots left{event.waitlisted ? ` · ${event.waitlisted} waitlisted` : ""}</small></span></div><div className="progress"><i style={{ width: `${Math.min(100, event.going / event.capacity * 100)}%` }} /></div></div>
    {event.viewerRsvpStatus && event.viewerCheckInCode && <section className={`event-ticket ${event.viewerCheckInStatus === "CHECKED_IN" ? "used" : ""}`}><div><span>{event.viewerRsvpStatus === "waitlisted" ? "WAITLIST TICKET" : "ADMIT ONE"}</span><b>{event.title}</b><small>{event.month} {event.day} · {event.time}</small></div><strong>{event.viewerCheckInCode}</strong><em>{event.viewerCheckInStatus === "CHECKED_IN" ? "CHECKED IN" : "SHOW AT ENTRY"}</em></section>}
