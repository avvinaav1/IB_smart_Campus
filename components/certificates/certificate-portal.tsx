...
export default function CertificatePortal({ events, appRole, preview = false, initialCode = "" }: { events: CampusEvent[]; appRole: AppRole; preview?: boolean; initialCode?: string }) {
  ...
  const [mode, setMode] = useState<"manage" | "verify">(globalManager && !initialCode ? "manage" : "verify");
  ...
  useEffect(() => {
    ...
    certificateRequest<{ canManage: boolean }>("/access").then(data => {
      ...
      if (!data.canManage) setMode("verify");
    }).catch(() => { if (active) setCanManage(false); });
    return () => { active = false; };
  }, [preview]);
  function choose(next: "manage" | "verify") { selected.current = true; setMode(next); }
  ...
  return <section className="certificate-portal-shell">
    ...
    {canManage && mode === "manage" ? <CertificateBuilder preview={preview} events={events} /> : <VerificationPortal initialCode={initialCode} embedded />}
  </section>;
}
