...
export default function CertificateBuilder({ preview = false, events = [] }: { preview?: boolean; events?: CampusEvent[] }) {
  ...
  const [emailSubject, setEmailSubject] = useState("Your {{event}} certificate"), [emailBody, setEmailBody] = useState("Hi {{name}},\n\nThank you for participating in {{event}}. Your certificate is ready.\n\nSee you on campus!");
  ...
  async function queue(actions: JobInput["requestedActions"]) {
    ...
    try {
      ...
      setJobId(data.jobId); setJobRows([]); setRowCursor(null); setTab("history"); setNotice("Batch queued and processing. Keep this Batch history tab open to keep it moving; reopen it here any time to pick back up.");
  ...
  return <section className="cert-builder">
    ...
    {tab === "delivery" && <div className="cert-delivery"><section className="cert-panel"><h2>Connect your attendees</h2><p>Match an email or exact username. Unmatched attendees still receive email when an address is available.</p><label>Event (optional)<select value={eventId} onChange={e => setEventId(e.target.value)}><option value="">Standalone certificate</option>{events.filter(e => e.canManageEvent).map(e => <option key={e.id} value={e.id}>{e.title}</option>)}</select></label><p className="cert-help">Choosing an event brands the certificate email with its banner alongside icebrkr, and unlocks the {"{{event}}"} variable for the subject/message below. Leave it as standalone to send icebrkr-branded email without an event banner.</p>{(["email", "username", "displayName"] as const).map(key => <label key={key}>{key === "displayName" ? "Recipient name column" : `${key} column`}<select value={mapping[key]} onChange={e => { setMapping(m => ({ ...m, [key]: e.target.value })); setMatches([]); }}><option value="">Not mapped</option>{headers.map(h => <option key={h}>{h}</option>)}</select></label>)}
      <label className="cert-check"><input type="checkbox" checked={internal} onChange={e => setInternal(e.target.checked)} />Save to profiles and Smart Campus Chat</label><label className="cert-check"><input type="checkbox" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} />Email a claim notice (no PNG attached — recipients sign in with this email to view it)</label><label className="cert-check"><input type="checkbox" checked={distinctAwards} onChange={e => setDistinctAwards(e.target.checked)} />Rows for the same recipient are distinct awards</label><button disabled={preview || busy} onClick={() => void checkMatches()}>Check internal matches</button>{matches.length > 0 && <div className="cert-match-report"><b>{matches.filter(m => m.status === "matched").length} of {matches.length} matched</b>{matches.filter(m => m.status !== "matched").slice(0, 20).map(m => <p key={m.rowNumber}>Row {m.rowNumber}: {m.reason}</p>)}</div>}</section>
      <section className="cert-panel"><h2>Your email, in your words</h2><p>Use any imported column as a variable, for example {"{{name}}"}. {selectedEvent ? <>Also use {"{{event}}"} for the event name (“{selectedEvent.title}”).</> : <>Select an event above to also use {"{{event}}"} for its name.</>}</p><label>Subject<input value={emailSubject} maxLength={200} onChange={e => setEmailSubject(e.target.value)} /></label><label>Message<textarea rows={9} value={emailBody} maxLength={10000} onChange={e => setEmailBody(e.target.value)} /></label>
        <small className="cert-mail-preview-label">PREVIEW · ROW {rowIndex + 1}</small>
        <div className="cert-mail-card">
          <div className="cert-mail-header"><img src="/logo.jpeg" alt="icebrkr" /></div>
          <div className="cert-mail-stripe"><span style={{ background: "#e8112e" }} /><span style={{ background: "#f0923a" }} /><span style={{ background: "#79b87d" }} /></div>
          {selectedEvent && <img src={selectedEvent.imageUrl} alt="" className="cert-mail-banner" />}
          {/* {selectedEvent && <div className="cert-mail-ribbon"><i style={{ color: "#e8112e" }}>■</i><i style={{ color: "#f0923a" }}>■</i><i style={{ color: "#79b87d" }}>■</i><span>{selectedEvent.title}</span></div>} */}
          <div className="cert-mail-body"><h3>{substitute(emailSubject, row.values, undefined, selectedEvent?.title)}</h3><p>{substitute(emailBody, row.values, undefined, selectedEvent?.title)}</p><div className="cert-mail-callout">🎓 Your certificate is ready - congratulations! 😊<br /><span>Create a free account or sign in with <b>{row.values[mapping.email] || "this email address"}</b> on IB Smart Campus to view and download it.</span></div><div className="cert-mail-cta"><span>Get my certificate →</span><small>Sends a one-time code to {row.values[mapping.email] || "this email"}</small></div></div>
          <div className="cert-mail-footer"><b>IB SMART CAMPUS</b><span>✦ Powered by icebrkr ✦</span></div>
        </div>
        {preview && <div className="cert-note" id="certificate-delivery-unavailable"><strong>Email delivery is unavailable in preview mode.</strong><p>Your attendees are ready to preview and export. To send them, the workspace administrator must configure the email service and database, and enable sign-in. Then sign in with your account.</p><p>You can download the certificates as a ZIP from Design &amp; personalize while delivery is being set up.</p></div>}
